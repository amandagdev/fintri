...
import type { FieldErrors, Quote, QuoteItem } from '@/features/quote/state'
import { formatDateForInput, getCurrentDate } from '@/lib/utils'

import { QuoteItems } from '../../quote-items'

interface QuoteDetailedFormProps {
  readonly data?: Quote
  ...
  readonly onItemsChange: (items: QuoteItem[]) => void
  readonly onDiscountChange: (e: React.ChangeEvent<HTMLInputElement>) => void
}

export function QuoteDetailedForm({
  data,
  errors,
  clients,
  selectedClient,
  ...
  return (
    <>
      ...
      <div className="mb-6">
        <QuoteItems items={items} onChange={onItemsChange} />
      </div>
